...
class AlarmController(
    ...
    fun stopAlarm(alarmId: Int) {
        ...
        ensureBackgroundThread {
            ...
            // the wake session ends here, record it for the sleep report
            alarm?.let { db.closeSleepRecord(it.id, System.currentTimeMillis()) }
    ...
    fun snoozeAlarm(alarmId: Int, snoozeMinutes: Int) {
        ...
        ensureBackgroundThread {
            ...
            if (alarm != null) {
                // the wake session continues until the final dismissal
                db.markSleepRecordSnoozed(alarm.id)
